...
export const applyProcessingResult = mutationGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    const firstNormalizedNote = normalizedResult.notes[0];
    const firstLine = firstNormalizedNote?.content.split("\n")[0]?.trim() ?? "";
    console.info("[notes.diagnostic] applyProcessingResult normalized", {
      sessionId: args.sessionId,
      noteCount: normalizedResult.notes.length,
      firstLine,
      containsLegacyTemplate: LEGACY_SUMMARY_MARKER_PATTERN.test(firstNormalizedNote?.content ?? ""),
      normalizer: "web/convex/lectureBuddy.ts:normalizeNoteSection",
    });
    const updatedAt = nowIsoString();
...
function normalizeNoteSection(
  ...
  if (LEGACY_SUMMARY_MARKER_PATTERN.test(normalizedContent)) {
    throw new Error(
      `Legacy summary-template note content is not allowed for session ${sessionId}.`,
    );
  }

  const compact = normalizeWhitespace(normalizedContent);
  const isNoNotesPayload = compact.toLowerCase().startsWith("no notes available");
  if (!isNoNotesPayload && !normalizedContent.includes(LECTURE_NOTES_V2_MARKER)) {
    throw new Error(
      `Notes payload missing required v2 marker for session ${sessionId}.`,
    );
  }
  ...
  return {
    ...
    startMs: readNumber(record.startMs, 0),
    endMs: readNumber(record.endMs, 0),
    contentFormat: "lecture-notes-v2" as const,
...
const LEGACY_SUMMARY_MARKER_PATTERN =
  /##\s*(summary|what was said|key points)|these notes are based on spoken lecture audio/i;
